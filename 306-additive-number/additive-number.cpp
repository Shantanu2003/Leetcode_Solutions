...
public:
    ...
    bool checkAddition(vector<string>& res, string a, string b, string c) {
    ...
    vector<string> additiveSequence(string num) {
        ...
        for (int i = 1; i <= l / 2; i++) {
            for (int j = 1; j <= (l - i) / 2; j++) {
                ...
                if (checkAddition(res, num.substr(0, i), num.substr(i, j),
                                  num.substr(i + j))) {
                    // add the substrings to the result vector and return it
                    res.push_back(num.substr(i, j));
                    res.push_back(num.substr(0, i));
                    return res;
                }
            }
        }

        // if no additive sequence is found, clear the result vector and return it
        res.clear();
        return res;
    }
